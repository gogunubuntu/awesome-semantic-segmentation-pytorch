...
class ADE20KSegmentationGho(SegmentationDataset):
    ...
    def __init__(self, root='../datasets/ade_gnd', split='test', mode=None, transform=None, **kwargs):
        super(ADE20KSegmentationGho, self).__init__(root, split, mode, transform, **kwargs)
...
def _get_ade20k_pairs(folder, mode='train'):
    ...
    if mode == 'train':
        ...
        mask_folder = os.path.join(folder, 'annotations/gho_train')
    else:
        img_folder = os.path.join(folder, 'images/validation')
        mask_folder = os.path.join(folder, 'annotations/gho_validation')
...
if __name__ == '__main__':
    train_dataset = ADE20KSegmentationGho()
