...
def resnet18_v1s(pretrained=False, root="~/.torch/models", **kwargs):
    model = ResNetV1b(BottleneckV1b, [2, 2, 2, 2], **kwargs)
    if pretrained:
        pass
        # from ..model_store import get_resnet_file

        # model.load_state_dict(
        #     torch.load(get_resnet_file("resnet18", root=root)), strict=False
        # )
    return model


def resnet34_v1s(pretrained=False, root="~/.torch/models", **kwargs):
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], **kwargs)
    if pretrained:
        pass
        # from ..model_store import get_resnet_file
